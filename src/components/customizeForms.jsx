...
function ColorInput({rgb,id, handleChange}) {
  return (
    <input type="color" id={id} value={`#${rgbHex(rgb)}`} onChange={(e) => {
...
function ColorPicker({styles,handleChange}) {
  return (
    ...
    <ColorInput id="backgroundColor" rgb={styles.get.backgroundColor} handleChange={handleChange}></ColorInput>
    </div>
    <div>
    <label htmlFor="color">Color</label>
    <ColorInput id="color" rgb={styles.get.color} handleChange={handleChange}></ColorInput>
    </div>
    </>
  )
}
